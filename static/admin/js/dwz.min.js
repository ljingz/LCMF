			...
			if (url) {
				if ($tab.hasClass("external")) {
					...
					$panel.ajaxUrl({
						 ...
						 * 添加type选项
						 */
						type:op.type || "POST", url:url, data:op.data, callback:function(response){
...
function navTabSearch(form, navTabId){
	 ...
	 * 添加type选项
	 */
	navTab.reload($form.attr('action'), {type: $form.attr('method'), data: $form.serializeArray(), navTabId:navTabId});
	return false;
}
/**
...
function dwzPageBreak(options){
	...
	if (op.rel) {
		...
		if (op.targetType == "dialog") {
			 ...
			 * 添加type选项
			 */
			if (form) navTab.reload($(form).attr("action"), {type: $(form).attr("method"), data: params, callback: op.callback});
		}
	}
}
/**
			...
			return this.each(function(){
				...
				$table.find("tr:first th[type]").each(function(i){
					...
					var field = {
						 ...
						 * 自定义html
						 */
						fieldHtml: $th.children("fieldHtml") || ""
					};
					fields.push(field);
				});
				...
				var $rowName = $table.attr('rowName') || "行";
				if (addButTxt) {
					var $rowNum = $('<span class="rowNum" style="float:left; margin:2px;">添加<input type="text" name="dwz_rowNum" value="1" size="2"/>'+$rowName+'</span>').insertBefore($table);
					...
					$addBut.click(function(){
						...
						var rowNum = 1;
						try{rowNum = parseInt($rowNum.children(":text").val())} catch(e){}
			...
			function tdHtml(field){
				...
				switch(field.type){
					...
					default:
						 ...
						 * 自定义html
						 */
						if(field.fieldHtml.length > 0){
							var fieldHtml = $('<span>'+field.fieldHtml.html()+'</span>');
							fieldHtml.find("input,select").attr({"name": field.name, "class": field.fieldClass}).val(field.defaultVal);
							html = fieldHtml.html();
						}else{
